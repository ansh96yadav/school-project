...
function TeacherHomeworkPage() {
 ...
 const fetchTasks = async () => {
  const res = await fetch("http://127.0.0.1:8000/homework");
...
useEffect(() => {
  ...
  const newTask = {...form, id: x, date_uploaded: currentDate, status:"Pending"}
    ...
    console.error("Failed to post homework");
  }
};

  const deleteTask = async (id) => {
    await fetch(`http://127.0.0.1:8000/homework/${id}`, {
    method: "DELETE"
  });
                ...
                {tasks.map((task) => (
                  <tr key={task.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2 flex gap-3"><button
                      ...
                      <td className='p-2'>{task.date_uploaded}</td>
